...
let cart = JSON.parse(localStorage.getItem('zhd_cart')) || [];
...
function updateCartUI() {
  ...
  cartSummary.innerText = `${totalQty} Products Added`;
  localStorage.setItem('zhd_cart', JSON.stringify(cart));
}
...
function generateCategories() {
  ...
  categories.forEach(category => {
    categoryContainer.innerHTML += `<button class="category-btn" data-category="${category}">${category}</button>`;
  });
...
function renderProducts() {
  ...
  filteredProducts.forEach((product) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-details">Pack: ${product.pack}</div>

      <div class="${product.stockStatus === 'IN STOCK' ? 'stock-in' : 'stock-out'}">
        ● ${product.stockStatus}
      </div>
      ...
      <div class="qty-row">
        ...
        >
          ${product.stockStatus === 'OUT OF STOCK' ? 'Out of Stock' : 'Add'}
        </button>
      </div>
    `;
...
function sendWhatsAppOrder() {
  ...
  cart.forEach(item => {
    message += `- ${item.brand} - ${item.name} - ${item.pack} | Qty: ${item.qty}%0A`;
  });
